<template>
    <div class="start-menu absolute flex flex-col bg-white bg-opacity-80 dark:bg-gray-600 dark:bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-md overflow-hidden duration-200"
        :class="{ 'show': isOpen }">
        ...
        <div class="flex flex-col p-6 flex-grow">
            ...
            <div class="flex-grow">
                <div class="flex items-center px-6 py-4">
                    ...
                    <button
                        class="flex items-center gap-2 text-2xs bg-white hover:bg-gray-50 dark:hover:bg-opacity-20 dark:bg-opacity-10 dark:text-white duration-100 px-3 py-1 rounded-sm shadow-sm">
                        <span>All apps</span>
                ...
                <div class="grid grid-cols-6">
                    <div v-for="(item, index) in apps" :key="index"
                        class="flex flex-col justify-center items-center gap-1 py-2 hover:bg-white hover:bg-opacity-50 active:bg-opacity-30 dark:hover:bg-opacity-10 dark:active:bg-opacity-5 rounded-md duration-150">
                        <img :src="item.icon" width="24" />
                        <span class="text-xs font-medium text-gray-800 dark:text-gray-200 text-center pt-2">
                            {{ item.label }}
                        </span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2">
                    <div v-for="(item, index) in recommendedApps" :key="index" class="flex gap-3 py-3 px-6">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-2xs font-medium text-gray-500 dark:text-white dark:text-opacity-30">{{
                            item.subLabel }}</span>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-black bg-opacity-5 dark:bg-opacity-10 px-12 py-4">
            <div class="user flex items-center gap-2">
                ...
                <div class="flex-grow"></div>
                <img src="@/assets/ui/power.png" class="rounded-full" :class="{ 'filter invert': !darkMode }"
                    width="14" />
            </div>
        </div>
